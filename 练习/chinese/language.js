...
;(function(){
	 ...
	 * @param classname
	 */
	function removeCls(ele,classname) {
		var oldCls = ele.getAttribute('class').split(' ');
		var rexCls = new RegExp(classname);
		var newCls = [];
		for (var i =1 ;i<oldCls.length; i++){
			if(!rexCls.test(oldCls[i])){
				newCls.push(oldCls[i]);
			}
		}
		ele.setAttribute('class',newCls.join(' '));
	}
	/**
...
// 转换为正体中文（繁体中文）
	window.toChinese = function(el) {
		...
		removeCls(document.querySelector(".simpleChinese"),'selected');
...
// 转换为简体中文
	window.toSimpleChinese = function(el) {
		...
		removeCls(document.querySelector(".Chinese"),'selected');
